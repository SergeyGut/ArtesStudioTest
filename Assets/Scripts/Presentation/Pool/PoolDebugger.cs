...
namespace Presentation.Pool
{
    public class PoolDebugger : MonoBehaviour
    {
        ...
        private IPiecePool<IPieceView> gemPool;
        ...
        private void Start()
        {
            ...
            {
                Debug.Log(
                    $"[PoolDebugger] Started. Baseline counts - Gems Active: {lastGemActiveCount}, Available: {lastGemAvailableCount}");
            }
        }
        ...
        private void CheckForGrowth()
        {
            ...
            {
                Debug.LogWarning(
                    $"[PoolDebugger] POOL GROWTH DETECTED! Active: {lastGemActiveCount} -> {currentActive} (+{activeGrowth}), Available: {lastGemAvailableCount} -> {currentAvailable} (+{availableGrowth})");
            }
        ...
        private void LogPoolStatus()
        {
            ...
            Debug.Log(
                $"[PoolDebugger] Pool Status - Gems Active: {gemPool.ActiveCount}, Available: {gemPool.AvailableCount}, Total: {gemPool.ActiveCount + gemPool.AvailableCount}");
            ...
            if (active.Count > 0)
            {
                ...
                foreach (var kvp in active)
                {
                    ...
                    {
                        Debug.LogWarning(
                            $"  {kvp.Key}: {kvp.Value} active (Gets: {getCount}, Releases: {releaseCount}) - POTENTIAL LEAK!");
                    }
                    else
                    {
        ...
        public void CheckForLeaks()
        {
            ...
            {
                Debug.LogError(
                    $"[PoolDebugger] POTENTIAL LEAK! Total gems ({totalGems}) is more than 2x expected ({expectedGems}). Active: {gemPool.ActiveCount}, Available: {gemPool.AvailableCount}");
            }
            else
            {
